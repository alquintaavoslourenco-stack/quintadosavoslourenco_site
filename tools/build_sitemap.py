...
def to_url(filepath):
    ...
    if filepath.endswith("/index.html"): return "/" + filepath[:-len("/index.html")] + "/"
    return "/" + filepath
...
def find_images_in_html(html_text, page_dir_url):
    ...
    for m in re.finditer(r'<img\b[^>]*\bsrc=["\']([^"\']+)["\']', html_text, flags=re.IGNORECASE):
        ...
        if src.startswith(("http://","https://")): abs_url = src
        elif src.startswith("//"): abs_url = "https:" + src
        elif src.startswith("/"): abs_url = BASE_URL + src
        else: abs_url = urljoin(page_dir_url.rstrip("/") + "/", src)
        if any(abs_url.lower().endswith(ext) for ext in IMG_EXT):
            imgs.add(abs_url)
    return sorted(imgs)
...
for root, dirs, files in os.walk("."):
    ...
    for f in files:
        if f.endswith(".html") and f not in IGNORE_FILES:
            html_files.append(os.path.join(root, f).lstrip("./"))

entries, image_map = [], {}
for f in sorted(html_files):
    url_path = to_url(f)
    page_url = BASE_URL + url_path
    lastmod = git_lastmod(f)[:10]
    entries.append((page_url, lastmod, changefreq_for(url_path), priority_for(url_path)))
    try:
        with open(f, "r", encoding="utf-8") as fh: txt = fh.read()
    except UnicodeDecodeError:
        with open(f, "r", encoding="latin-1", errors="ignore") as fh: txt = fh.read()
    dir_url = BASE_URL if "/" not in f else (BASE_URL + "/" + f.rsplit("/",1)[0])
    imgs = find_images_in_html(txt, dir_url)
    if imgs: image_map[page_url] = imgs

seen, unique_pages = set(), []
for e in entries:
    if e[0] in seen: continue
    seen.add(e[0]); unique_pages.append(e)
